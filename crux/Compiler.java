package crux;
import java.io.File;
import java.io.FileNotFoundException;
import java.io.FileReader;
import java.io.IOException;
import java.io.PrintStream;

public class Compiler {
    public static String studentName = "James V. Soukup";
    public static String studentID = "48114397";
    public static String uciNetID = "jsoukup";
    
    public static void main(String[] args)
    {
        String sourceFilename = args[0];
        
        // Determine the test number
        // sourceFilename: ".\tests\testXX.crx"
        int testNum = Integer.parseInt(sourceFilename.substring(12, 14));
        
        // Create the output file
        String resFileName = ".\\results\\test"+String.format("%02d",testNum)+".txt";
        File resFile = new File(resFileName);
        
        Scanner s = null;
        try 
        {
            s = new Scanner(new FileReader(sourceFilename));
        } 
        catch (IOException e) 
        {
            e.printStackTrace();
            System.err.println("Error accessing the source file: \"" + sourceFilename + "\"");
            System.exit(-2);
        }
        
        try 
        {
			PrintStream ps = new PrintStream(resFile);

			Parser p = new Parser(s);
			p.parse();
			if (p.hasError()) 
			{
			    System.out.println("Error parsing file.");
			    System.out.println(p.errorReport());
			    ps.println("Error parsing file.");
			    ps.println(p.errorReport());
			    System.exit(-3);
			}
			System.out.println("Crux program successfully parsed.");
			ps.println("Crux program successfully parsed.");
		} 
        catch (FileNotFoundException e) 
        {
			e.printStackTrace();
			System.exit(-2);
		}
    }
}
    
